...
import {
  ...
  DebateResponse,
} from '@/lib/matching-algorithm-debate'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Get session with debate responses
    ...
    // Parse responses
    const responses: DebateResponse[] = typeof session.responses_v2 === 'string'
    ...
    // 3. Calculate matching using debate algorithm
    const result = processDebateResponses(responses, candidates)

    // 4. Save results as JSON in session
    const resultsJson = JSON.stringify({
      rankings: result.rankings.map(r => ({
        ...
        matchingPosturasCount: r.matchingPosturas.length
      })),
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get session with debate results and responses
    const sessionResult = await sql`
      SELECT
        id,
        responses_v2,
    ...
    const rankings = storedResults.rankings.map((r: any) => {
      ...
      return {
        ...
        percentage: r.percentage,
        dimensionBreakdown: r.dimensionBreakdown,
        matchingPosturasCount: r.matchingPosturasCount
      }
